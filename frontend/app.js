...
function buildLayout() {
  ...
  hosts.forEach((ip, hi) => {
    ...
    const activeDevIp = deviceActiveIp[ip] || ip;
    makeNode({ id: `dev-${ip}`, label, type: 'device', ip, activeIp: activeDevIp, aliases: meta.aliases, mac: meta.mac }, dx, dy);
    ...
    svcs.forEach((svc, si) => {
      ...
      const activeIp = deviceActiveIp[ip] || ip;
      const url = o.url || `http://${activeIp}:${port}`;
...
function makeNode(cfg, x, y) {
  ...
  const ipSubtitle = (cfg.type === 'device' && cfg.activeIp)
    ? `<div class="node-ip-subtitle">${cfg.activeIp}</div>`
    : '';
  ...
  if (cfg.type === 'device') {
    ...
    const allIps = [cfg.ip, ...(cfg.aliases || [])];
    if (allIps.length > 1) {
      el.style.cursor = 'pointer';
      el.addEventListener('click', e => {
        if (e.target.closest('.node-actions')) return;
        const current = deviceActiveIp[cfg.ip] || cfg.ip;
        const idx = allIps.indexOf(current);
        deviceActiveIp[cfg.ip] = allIps[(idx + 1) % allIps.length];
        buildLayout();
      });
    }
  }
